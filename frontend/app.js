...
function renderFeedback(containerId, feedback) {
  ...
  const map = {
    ...
    weaknesses: { label: 'Areas to Improve', cls: 'neg' },
...
function getGrade(score) {
  ...
  return { grade: 'Below Average — Revamp Needed', color: '#F87171' };
}
...
function downloadPDF() {
  if (!generatedPDFBytes) { showToast('No PDF available.'); return; }
...
async function scoreResume() {
  if (!selectedPDFFile) { showToast('Please select a PDF file first.'); return; }
...
async function loadLeaderboard() {
  ...
  try {
    ...
    body.innerHTML = `<div class="lb-empty">${err.message}</div>`;
  }
}
...
function showCloudinaryFallback(body, url) {
  body.innerHTML = `
    <div class="pdf-fallback" style="display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; padding: 2.5rem 1.5rem;">
      <div class="pdf-fallback-icon" style="font-size:3rem; margin-bottom:1rem; animation: pulse 2s infinite;"></div>
